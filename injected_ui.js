...
(function () {
  ...
  Object.assign(blackoutLayer.style, {
    ...
    zIndex: "2147483640",
    background:
      "radial-gradient(ellipse at center, transparent 50%, rgba(0,0,0,0.95) 100%)",
    opacity: "0",
  ...
  const G_POS_SAFE = 4.5;
  const G_NEG_SAFE = -1.5;
  const G_POS_FULL = 8.0;
  const G_NEG_FULL = -4.0;
  const STRESS_RATE = 0.9;
  const RECOVERY_RATE = 0.45;
  const LOCK_ON = 0.98;
  const LOCK_OFF = 0.5;

  let gStress = 0;
  let isLockedOut = false;
  let stressIsRed = false;
  ...
  window.addEventListener("wheel", swallowInput, {
    capture: true,
    passive: false,
  });
  ...
  function gameLoop() {
    ...
    if (blackoutToggle && blackoutToggle.checked && gStress > 0.01) {
      ...
      blackoutLayer.style.background = `radial-gradient(ellipse at center, transparent ${visionRadius}%, ${tint} 100%)`;
